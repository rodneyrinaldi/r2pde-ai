...
export async function manifestDeleteCommand(): Promise<void> {
  ...
  const files = fs.readdirSync(paths.manifests).filter(f => f.endsWith('.md'));
  if (files.length === 0) {
    logInfo('No manifests found.');
    return;
  }
  logInfo('Select the artifact to permanently delete. This action cannot be undone and will remove the file from disk.');
  const selected = await select({
    message: 'Select a manifest to delete:',
  ...
  const doDelete = await confirm({ message: `Delete manifest '${name}'?`, default: false });
  if (!doDelete) {
    logInfo('Operation cancelled.');
    return;
  }
  const filePath = path.join(paths.manifests, selected);
  fs.unlinkSync(filePath);
  // Step 5 — Log entry
  const logPath = path.join(paths.logs, 'pde.log.md');
  const logLine = `- ${new Date().toISOString()} | manifest:delete | ${selected} | deleted\n`;
  fs.ensureFileSync(logPath);
  fs.appendFileSync(logPath, logLine, { encoding: 'utf8' });
  // Step 6 — Git commit if enabled
  const config = loadConfig(cwd);
  gitAddAndCommit(cwd, `feat(manifest): remove ${name}`, config);
  // Step 7 — Final output
  logSuccess(`manifest '${name}' deleted.`);
}
